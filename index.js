...
const invocationName = "fredonia assistant scheduler";
...
const getDeadlines_Handler =  {
    canHandle(handlerInput) {
        const request = handlerInput.requestEnvelope.request;
        return request.type === 'IntentRequest' && request.intent.name === 'getDeadlines' ;
    },
    handle(handlerInput) {
        const request = handlerInput.requestEnvelope.request;
        const responseBuilder = handlerInput.responseBuilder;
        ...
        //   SLOT: deadlines 
        if (slotValues.deadlines.heardAs == 'midterm grades')
        {
            slotStatus = 'Midterm grades are available 10/24/19';
        }
         if (slotValues.deadlines.heardAs == 'first day of classes')
        {
            slotStatus = 'The first day of classes are 08/26/19 ';
        }
        if (slotValues.deadlines.heardAs == 'orientation')
        {
            slotStatus = 'Orientation starts 08/23/19';
        }
         if (slotValues.deadlines.heardAs == 'residence halls open')
        {
            slotStatus = 'Residence Halls open 08/24/19 ';
        }
        if (slotValues.deadlines.heardAs == 'final day to drop courses')
        {
            slotStatus = 'The last day to add courses are 08/30/19';
        }
         if (slotValues.deadlines.heardAs == 'final day to add courses')
        {
            slotStatus = 'The final day to drop courses are 07/4/19 ';
        }
        if (slotValues.deadlines.heardAs == 'graduate registration')
        {
            slotStatus = 'Graduate registration begins 10/25/19 ';
        }
        if (slotValues.deadlines.heardAs == 'academic advising')
        {
            slotStatus = 'Academic Advising & Course Selection for Spring 2020 Begins on 10/28/19';
        }
        if (slotValues.deadlines.heardAs == 'final day to withdraw from university')
        {
            slotStatus = 'The final day to withdraw from the university for the fall semester is 10/8/19';
        }
         if (slotValues.deadlines.heardAs == 'last day of classes')
        {
            slotStatus = 'The last day of classes for the fall semester is 12/13/19 ';
        }
        if (slotValues.deadlines.heardAs == 'final exam week')
        {
            slotStatus = 'Final exam week starts on 12/16/19 and ends on 12/20/19';
        }
         if (slotValues.deadlines.heardAs == 'final grades available')
        {
            slotStatus = 'Final grades are available 12/27/19 at 3:00 PM';
        }
        if (slotValues.deadlines.heardAs == 'final day to apply for graduation')
        {
            slotStatus = 'The final day to apply for December graduation is 07/11/19 ';
        }
        
        // If slot is empty
        if (slotValues.deadlines.heardAs && slotValues.deadlines.heardAs !== '') {
        //    slotStatus += ' slot deadlines was heard as ' + slotValues.deadlines.heardAs + '. ';
        } else {
          //  slotStatus += 'slot deadlines is empty. ';
        }
        if (slotValues.deadlines.ERstatus === 'ER_SUCCESS_MATCH') {
           // slotStatus += 'a valid ';
            if(slotValues.deadlines.resolved !== slotValues.deadlines.heardAs) {
                slotStatus += 'synonym for ' + slotValues.deadlines.resolved + '. '; 
                } else {
               // slotStatus += 'match. '
            } // else {
                //
        }
        if (slotValues.deadlines.ERstatus === 'ER_SUCCESS_NO_MATCH') {
          //  slotStatus += 'which did not match any slot value. ';
            console.log('***** consider adding "' + slotValues.deadlines.heardAs + '" to the custom slot type used by slot deadlines! '); 
        }

        if( (slotValues.deadlines.ERstatus === 'ER_SUCCESS_NO_MATCH') ||  (!slotValues.deadlines.heardAs) ) {
           // slotStatus += 'A few valid values are, ' + sayArray(getExampleSlotValues('getDeadlines','deadlines'), 'or');
        }
...
const GetRemoteDataHandler = {
  ...
  async handle(handlerInput) {
    ...
    await getRemoteData('https://events.fredonia.edu/api/2/events?type=105894&pp=100&page=1')
      .then((response) => {
        const data = JSON.parse(response);
     
        
        outputSpeech = `There are  ${data.events.length} Student Life events going on today. `
        for (let i = 0; i < data.events.length; i++) {
          if (i === 0) {
            //first record
            outputSpeech = outputSpeech + 'Student Events going on today are: ' + data.events[i].event.title + ', '
          } else if (i === data.events.length) {
            //last record
           // outputSpeech = outputSpeech + 'and ' + data.events[i].event.title  + '.'
          } else {
            //middle record(s)
            outputSpeech = outputSpeech + data.events[i].event.title + ' at the ' + data.events[i].event.location_name + ' at ' +  (data.events[i].event.event_instances[0].event_instance.start) + ', '
          }
        }
      })
      .catch((err) => {
        //set an optional error message here
        outputSpeech = 'This is an error message from the dynamic events intent'
      });
